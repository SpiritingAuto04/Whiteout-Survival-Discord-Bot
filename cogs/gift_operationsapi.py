                            ...
                            if invalid_codes:
                                for invalid_code in invalid_codes:
                                    try:
                                        ...
                                        async with session.delete(self.api_url, json=data,
                                        ...
                                        if auto_alliances:
                                            for alliance in auto_alliances:
                                                try:
                                                    ...
                                                    if gift_operations:
                                                        await gift_operations.use_giftcode_for_alliance(alliance[0],
                                                                                                        code)
                                                        await asyncio.sleep(1)
                                                    else:
                                                        traceback.print_exc()
                ...
                async with session.post(self.api_url, json=data, headers=headers) as response:
                    if response.status == 200:
                        ...
                        if result.get('success'):
                            self.cursor.execute("INSERT OR IGNORE INTO gift_codes (giftcode, date) VALUES (?, ?)",
