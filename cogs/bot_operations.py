...
class BotOperations(commands.Cog):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        match custom_id:
            ...
            case "alliance_control_messages":
                try:
            ...
            case ["assign_alliance", "add_admin", "remove_admin", "main_menu", "bot_status", "bot_settings"]:
                try:
                    if custom_id == "assign_alliance":
                                    ...
                                    try:
                                        ...
                                        for alliance_id, name in alliances:
                                            with sqlite3.connect('db/users.sqlite') as users_db:
                                                ...
                                                cursor.execute("SELECT COUNT(*) FROM users WHERE alliance = ?",
                                                               (alliance_id,))
                        ...
                        try:
                            self.settings_cursor.execute("SELECT is_initial FROM admin WHERE id = ?",
                                                         (interaction.user.id,))
                            result = self.settings_cursor.fetchone()
                            ...
                            async def admin_callback(select_interaction: discord.Interaction):
                                try:
                                    ...
                                    async def confirm_callback(button_interaction: discord.Interaction):
                                        try:
                                            self.settings_cursor.execute("DELETE FROM adminserver WHERE admin = ?",
                                                                         (selected_admin_id,))
                                            self.settings_cursor.execute("DELETE FROM admin WHERE id = ?",
                                                                         (selected_admin_id,))
                                            self.settings_db.commit()
                            ...
                            async def select_callback(select_interaction: discord.Interaction):
                                try:
                                    ...
                                    async def confirm_callback(confirm_interaction: discord.Interaction):
                                        try:
                                            success = await self.confirm_permission_removal(int(admin_id), int(alliance_id),
                                                                                            confirm_interaction)
                ...
                try:
                    ...
                    for admin_id, is_initial in admins:
                        try:
                            ...
                            admin_list_embed.add_thumbnail(url=admin_avatar)
    ...
    async def confirm_permission_removal(self, admin_id: int, alliance_id: int,
